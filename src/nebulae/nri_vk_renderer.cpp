...
Renderer::~Renderer()
{
    ...
    vkDestroyDescriptorSetLayout(m_device->GetNativeHandle(), m_descriptorSetLayout, nullptr);
    vkDestroyDescriptorPool(m_device->GetNativeHandle(), m_descriptorPool, nullptr);
...
{
    auto nextFrameTimestamp = std::chrono::high_resolution_clock::now();
    auto durationSinceLastFrame = nextFrameTimestamp - m_frameTimestamp;
    m_frameTimestamp = nextFrameTimestamp;

    static auto timestampPrintInterval = std::chrono::seconds(1);
    static auto timeSinceLastTimestampPrint = std::chrono::duration<float>::zero();
    timeSinceLastTimestampPrint += durationSinceLastFrame;
    if (timeSinceLastTimestampPrint >= timestampPrintInterval)
    {
        auto fps = 1.0f / std::chrono::duration<float>(durationSinceLastFrame).count();
        WARP_LOG_TRACE(LOGGER_NRI, "Frame time: {} ms ({} fps)",
                       std::chrono::duration<float, std::chrono::milliseconds::period>(durationSinceLastFrame).count(),
                       fps);
        timeSinceLastTimestampPrint = std::chrono::duration<float>::zero();
    }
    ...
    NRI_VK_CHECK_RESULT(vkWaitForFences(m_device->GetNativeHandle(), 1, &inflightFence, VK_TRUE, UINT64_MAX),
    ...
    UpdateGlobalUniformBuffer(swapchainImageIndex);

    // after waiting for fences we need to manually reset them
    // also important to only reset fences if no resize should have occured during this frame! So after acquiring an
    // image!
    NRI_VK_CHECK_RESULT(vkResetFences(m_device->GetNativeHandle(), 1, &inflightFence),
    ...
    NRI_VK_CHECK_RESULT(
        vkQueueSubmit(m_device->GetQueue(vk::EDeviceQueueType::Universal), 1, &submitInfo, inflightFence),
    ...
    NRI_VK_CHECK_RESULT(vkQueuePresentKHR(m_device->GetQueue(vk::EDeviceQueueType::Universal), &presentInfo),
...
void Renderer::RecordCommandBuffer(VkCommandBuffer commandBuffer, uint32_t swapchainImageIndex)
{
    ...
    NRI_VK_CHECK_RESULT(vkBeginCommandBuffer(commandBuffer, &commandBufferBeginInfo), "Failed to begin command buffer");
    {
        ...
        // Bind vertex & index buffer information to the pipe
        {
            ...
            std::ranges::transform(m_vertexAttributeBuffers, nativeVertexBuffers.begin(),
                                   [](const Arc<vk::NriBuffer>& buffer)
                                   {
                                       return buffer->GetNativeHandle();
                                   });
            vkCmdBindVertexBuffers(commandBuffer, 0, NumVertexAttributes, nativeVertexBuffers.data(),
                                   dummyOffsets.data());

            VkIndexType indexType = m_quadVertexCollection.indexAttribute.GetFormat() == VK_FORMAT_R16_UINT
                                        ? VK_INDEX_TYPE_UINT16
                                        : VK_INDEX_TYPE_UINT32;
            vkCmdBindIndexBuffer(commandBuffer, m_indexBuffer->GetNativeHandle(), 0, VK_INDEX_TYPE_UINT16);

            auto bindDescriptorSetInfo = NRI_VK_STRUCT(VkBindDescriptorSetsInfo);
            bindDescriptorSetInfo.stageFlags = VK_SHADER_STAGE_VERTEX_BIT;
            bindDescriptorSetInfo.layout = m_triangleLayout;
            bindDescriptorSetInfo.firstSet = 0;
            bindDescriptorSetInfo.descriptorSetCount = 1;
            bindDescriptorSetInfo.pDescriptorSets = &m_descriptorSets.at(swapchainImageIndex);
            bindDescriptorSetInfo.dynamicOffsetCount = 0;
            bindDescriptorSetInfo.pDynamicOffsets = nullptr;
            vkCmdBindDescriptorSets2(commandBuffer, &bindDescriptorSetInfo);
        }
...
void Renderer::UpdateGlobalUniformBuffer(uint32_t swapchainImageIndex)
{
    ...
    ubo.modelMatrix = glm::rotate(glm::mat4(1.0f), time * glm::radians(90.0f), glm::vec3(0.4f, 0.0f, 1.0f));
    ...
    ubo.projectionMatrix = glm::perspective(
        glm::radians(45.0f), static_cast<float>(m_swapchain->GetWidth()) / m_swapchain->GetHeight(), 0.1f, 10.0f);
    std::memcpy(m_globalUniformBuffers.at(swapchainImageIndex)->GetPersistentlyMappedMemory(), &ubo, sizeof(ubo));
}

void Renderer::InitInstance()
{
    m_instance = Arc<vk::NriInstance>::Make();
    if (!m_instance->Init(vk::NriInstanceCreateInfo{.appName = "Warp test application",
                                                    .engineName = "Warp Engine",
                                                    .appVersion = VK_MAKE_API_VERSION(1, 0, 0, 0),
                                                    .engineVersion = VK_MAKE_API_VERSION(1, 0, 0, 0),
                                                    .window = m_info.window}))
    {
        WARP_ASSERT(false, "Failed to create NriInstance");
    }
}

void Renderer::InitDevice()
{
    m_device = Arc<vk::NriDevice>::Make();
    if (!m_device->Init(vk::NriDeviceCreateInfo{
            .instance = m_instance,
            .window = m_info.window,
        }))
    {
        WARP_ASSERT(false, "Failed to create NriDevice");
    }
}

void Renderer::InitSwapchain()
{
    m_swapchain = Arc<vk::NriSwapchain>::Make();
    if (!m_swapchain->Init(vk::NriSwapchainCreateInfo{
            .device = m_device,
            .numSwapchainImages = 3,
        }))
    {
        WARP_ASSERT(false, "Failed to create NriSwapchain");
    }
}
...
void Renderer::InitTriangleShaderModules()
{
    static auto CreateShaderModule = [nativeDevice = m_device->GetNativeHandle()](std::span<const uint32_t> spirvWords)
    {
        ...
        NRI_VK_CHECK_RESULT(vkCreateShaderModule(nativeDevice, &createInfo, nullptr, &shaderModule),
    ...
    ShaderCompilerOutput vsHelloTriangleSpirv =
        shaderLibrary->CompileFromLibrary(EShaderLang::Slang, "hello_triangle_ubo.slang", "vertexMain", {});
    ShaderCompilerOutput fsHelloTriangleSpirv =
        shaderLibrary->CompileFromLibrary(EShaderLang::Slang, "hello_triangle_ubo.slang", "fragmentMain", {});
...
void Renderer::InitGraphicsTriangleRenderPass()
{
    ...
    attachmentInfo.flags =
        0; // https://docs.vulkan.org/refpages/latest/refpages/source/VkAttachmentDescriptionFlagBits.html
    ...
    subpassInfo.viewMask =
        0; // bitfield of view indices describing which views are active during rendering, when it is not 0
    ...
    subpassDependencyInfo.srcSubpass =
    ...
    NRI_VK_CHECK_RESULT(
        vkCreateRenderPass2(m_device->GetNativeHandle(), &renderPassInfo, nullptr, &m_triangleRenderPass),
        "Failed to create render pass");
}

void Renderer::InitGraphicsTrianglePipe()
{
    // when compiled to SPIR-V, shader module will keep entrypoint name as 'main' to match defaults found in other
    // shading langauges such as GLSL. It is also valid in a single SPIR-V binary to have 'main' for two different
    // stages.
    ...
    std::array vertexBindingDescriptions = {
        m_quadVertexCollection.GetBindingDescription(EVertexAttributeIndex::Position, true),
    ...
    pipeRasterizationInfo.frontFace = VK_FRONT_FACE_CLOCKWISE;
    ...
    NRI_VK_CHECK_RESULT(
        vkCreatePipelineLayout(m_device->GetNativeHandle(), &pipeLayoutInfo, nullptr, &m_triangleLayout),
    ...
    NRI_VK_CHECK_RESULT(vkCreateGraphicsPipelines(m_device->GetNativeHandle(),
                                                  ...
                                                  1, &graphicsPipeCreateInfo, nullptr, &m_trianglePipe),
...
void Renderer::InitFramebuffers()
{
    ...
    for (uint32_t swapchainImageIndex = 0; swapchainImageIndex < m_swapchain->GetNumSwapchainImages();
         ++swapchainImageIndex)
    {
        ...
        NRI_VK_CHECK_RESULT(vkCreateFramebuffer(m_device->GetNativeHandle(), &framebufferInfo, nullptr,
                                                &m_swapchainFramebuffers.at(swapchainImageIndex)),
                            "Failed to create framebuffer for swapchain image {}", swapchainImageIndex);
    }
}
...
void Renderer::InitTriangleCommandBuffers()
{
    ...
    NRI_VK_CHECK_RESULT(
        vkAllocateCommandBuffers(m_device->GetNativeHandle(), &commandBufferInfo, m_commandBuffers.data()),
...
void Renderer::InitSyncPrimitives()
{
    ...
    for (uint32_t inflightFrameIndex = 0; inflightFrameIndex < numFramesInFlight; ++inflightFrameIndex)
    {
        ...
        NRI_VK_CHECK_RESULT(vkCreateSemaphore(m_device->GetNativeHandle(), &imageAvailableSemaphoreInfo, nullptr,
                                              &m_imageAvailableSemaphores.at(inflightFrameIndex)),
                            "Failed to create image available semaphore");

        auto renderFinishedSemaphoreInfo = NRI_VK_STRUCT(VkSemaphoreCreateInfo);
        NRI_VK_CHECK_RESULT(vkCreateSemaphore(m_device->GetNativeHandle(), &renderFinishedSemaphoreInfo, nullptr,
                                              &m_renderFinishedSemaphores.at(inflightFrameIndex)),
        ...
        NRI_VK_CHECK_RESULT(vkCreateFence(m_device->GetNativeHandle(), &inflightFenceInfo, nullptr,
                                          &m_inflightFences.at(inflightFrameIndex)),
...
void Renderer::InitQuadVertexBuffers()
{
    ...
    VkCommandBuffer bufferCopyCommandBuffer = m_device->AllocateSingleSubmitCmdBuffer(CopyQueueType);
    m_device->BeginDebugLabel(bufferCopyCommandBuffer, "Copy vertex attribute buffers",
    ...
    m_device->EndAndSubmitCmdBuffer(CopyQueueType, bufferCopyCommandBuffer);
    m_device->WaitIdle();
    m_device->DeallocateSingleSubmitCmdBuffer(CopyQueueType, bufferCopyCommandBuffer);
}
...
void Renderer::InitQuadIndexBuffer()
{
    ...
    // allocate GPU local buffers for each vertex attribute and copy data from staging buffers to them
    m_indexBuffer = Arc<vk::NriBuffer>::Make(
        vk::NriBufferInfo{.device = m_device,
    ...
    NRI_VK_CHECK_RESULT(
        vkAllocateCommandBuffers(m_device->GetNativeHandle(), &commandBufferAllocInfo, &bufferCopyCommandBuffer),
    ...
    NRI_VK_CHECK_RESULT(vkBeginCommandBuffer(bufferCopyCommandBuffer, &beginInfo),
                        "Failed to begin command buffer for buffer copy operations");
    ...
    m_device->EndDebugLabel(bufferCopyCommandBuffer);
    NRI_VK_CHECK_RESULT(vkEndCommandBuffer(bufferCopyCommandBuffer),
                        "Failed to end command buffer for buffer copy operations");

    auto submitInfo = NRI_VK_STRUCT(VkSubmitInfo);
    submitInfo.commandBufferCount = 1;
    submitInfo.pCommandBuffers = &bufferCopyCommandBuffer;
    NRI_VK_CHECK_RESULT(vkQueueSubmit(m_device->GetQueue(CopyQueueType), 1, &submitInfo, VK_NULL_HANDLE),
                        "Failed to submit queue commands");
    m_device->WaitIdle();
}
...
void Renderer::InitDescriptorPool()
{
    ...
    descriptorPoolSize.descriptorCount =
        m_info.numFramesInFlight; // number of descriptors to allocate of the specified type

    auto descriptorPoolInfo = NRI_VK_STRUCT(VkDescriptorPoolCreateInfo);
    descriptorPoolInfo.flags = 0;
    descriptorPoolInfo.maxSets = m_info.numFramesInFlight;
    descriptorPoolInfo.poolSizeCount = 1;
    descriptorPoolInfo.pPoolSizes = &descriptorPoolSize;
    NRI_VK_CHECK_RESULT(
        vkCreateDescriptorPool(m_device->GetNativeHandle(), &descriptorPoolInfo, nullptr, &m_descriptorPool),
...
void Renderer::InitDescriptorSets()
{
    ...
    auto descriptorSetLayoutInfo = NRI_VK_STRUCT(VkDescriptorSetLayoutCreateInfo);
    descriptorSetLayoutInfo.flags = 0;
    descriptorSetLayoutInfo.bindingCount = 1;
    descriptorSetLayoutInfo.pBindings = &descriptorSetLayoutBinding;
    NRI_VK_CHECK_RESULT(vkCreateDescriptorSetLayout(m_device->GetNativeHandle(), &descriptorSetLayoutInfo, nullptr,
                                                    &m_descriptorSetLayout),
                        "Failed to create descriptor set layout");

    m_descriptorSets.resize(m_info.numFramesInFlight);
    std::vector<VkDescriptorSetLayout> layouts(m_info.numFramesInFlight, m_descriptorSetLayout);
    auto descriptorSetAllocInfo = NRI_VK_STRUCT(VkDescriptorSetAllocateInfo);
    descriptorSetAllocInfo.descriptorPool = m_descriptorPool;
    descriptorSetAllocInfo.descriptorSetCount = m_info.numFramesInFlight;
    descriptorSetAllocInfo.pSetLayouts = layouts.data();
    NRI_VK_CHECK_RESULT(
        vkAllocateDescriptorSets(m_device->GetNativeHandle(), &descriptorSetAllocInfo, m_descriptorSets.data()),
        "Failed to allocate descriptor sets");

    for (uint32_t i = 0; i < m_info.numFramesInFlight; ++i)
    {
        VkDescriptorBufferInfo bufferInfo{};
        bufferInfo.buffer = m_globalUniformBuffers.at(i)->GetNativeHandle();
        bufferInfo.offset = 0;
        bufferInfo.range = sizeof(GlobalUniformBuffer);

        auto descriptorWrite = NRI_VK_STRUCT(VkWriteDescriptorSet);
        ...
        vkUpdateDescriptorSets(m_device->GetNativeHandle(), 1, &descriptorWrite, 0, nullptr);
    }
}
...
{
    const uint32_t numFramesInFlight = m_info.numFramesInFlight;
    m_globalUniformBuffers.resize(numFramesInFlight);

    static constexpr VkDeviceSize GlobalUniformBufferSize = sizeof(GlobalUniformBuffer);

    for (uint32_t i = 0; i < numFramesInFlight; ++i)
    {
        m_globalUniformBuffers[i] = Arc<vk::NriBuffer>::Make(
            vk::NriBufferInfo{.device = m_device,
                              .usage = VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT,
                              .sizeInBytes = GlobalUniformBufferSize,
                              .memoryAllocationFlags = VMA_ALLOCATION_CREATE_MAPPED_BIT |
