    ...
    : m_nativeHandle(nativeHandle)
{
    ...
    NRI_VK_CHECK_RESULT(
        vkEnumerateDeviceExtensionProperties(GetNativeHandle(), nullptr, &extensionPropertyCount, nullptr),
        "Failed to enumerate device extensions");
}

NriPhysicalDeviceSurfaceProperties NriPhysicalDevice::QuerySurfaceProperties(VkSurfaceKHR nativeSurface)
{
    WARP_ASSERT(nativeSurface != VK_NULL_HANDLE,
                "Invalid native surface handle provided to query physical device surface properties");
    ...
    // query surface capabilities
    {
        ...
        NRI_VK_CHECK_RESULT(
            vkGetPhysicalDeviceSurfaceCapabilities2KHR(nativeHandle, &surfaceInfo, &surfaceCapabilities2KHR),
    ...
    // Query surface formats available on this physical device
    {
        ...
        NRI_VK_CHECK_RESULT(
            vkGetPhysicalDeviceSurfaceFormatsKHR(nativeHandle, nativeSurface, &numSurfaceFormats, nullptr),
            "Failed to query physical device surface formats");

        surfaceProperties.availableSurfaceFormats.resize(numSurfaceFormats);
        NRI_VK_CHECK_RESULT(vkGetPhysicalDeviceSurfaceFormatsKHR(nativeHandle, nativeSurface, &numSurfaceFormats,
                                                                 surfaceProperties.availableSurfaceFormats.data()),
                            "Failed to query physical device surface formats");
    }
    ...
    // Query available present modes
    {
        ...
        NRI_VK_CHECK_RESULT(
            vkGetPhysicalDeviceSurfacePresentModesKHR(nativeHandle, nativeSurface, &numPresentModes, nullptr),
            "Failed to query physical device surface present modes");

        surfaceProperties.availablePresentModes.resize(numPresentModes);
        NRI_VK_CHECK_RESULT(vkGetPhysicalDeviceSurfacePresentModesKHR(nativeHandle, nativeSurface, &numPresentModes,
                                                                      surfaceProperties.availablePresentModes.data()),
                            "Failed to query physical device surface present modes");
    }
    return surfaceProperties;
}

bool NriPhysicalDevice::IsSurfaceSupportedByQueueFamily(
    VkSurfaceKHR nativeSurface, const NriPhysicalDeviceQueueFamilyInformation& queueFamilyInfo) const
{
    // Check whether this queue family supports presentation to the surface provided
    VkBool32 isSurfaceSupported = VK_FALSE;
    NRI_VK_CHECK_RESULT(vkGetPhysicalDeviceSurfaceSupportKHR(GetNativeHandle(), queueFamilyInfo.queueFamilyIndex,
                                                             nativeSurface, &isSurfaceSupported),
...
void NriPhysicalDevice::QuerySupportedExtensions()
{
    ...
    NRI_VK_CHECK_RESULT(vkEnumerateDeviceExtensionProperties(GetNativeHandle(), nullptr, &extensionPropertyCount,
                                                             m_supportedDeviceExtensionProperties.data()),
                        "Failed to enumerate device extensions");

    // transform plain unordered access array into a look-up hash-set to easily query for all recognized extensions an
    // engine might require
    m_supportedDeviceExtensionLUT.clear();
    m_supportedDeviceExtensionLUT.insert_range(m_supportedDeviceExtensionProperties |
                                               std::views::transform(
...
void NriPhysicalDevice::QueryQueueFamilyProperties()
{
    ...
    // Starting from Vulkan 1.4 extension VK_KHR_global_priority (or VK_EXT_global_priority_query) is ratified in core
    // API Check whether global priority API is available on this device
    bool hasGlobalPriorityProperties =
        GetApiVersion() >= EApiVersion::Vk_1_4 || IsExtensionSupported(VK_KHR_GLOBAL_PRIORITY_EXTENSION_NAME);
    ...
    for (uint32_t queueFamilyIndex = 0; queueFamilyIndex < numProperties; ++queueFamilyIndex)
    {
        ...
        const VkQueueFamilyProperties& srcFamilyProperties =
            physicalDeviceQueueFamilyProperties.at(queueFamilyIndex).queueFamilyProperties;

        queueFamilyInfo.queueFamilyIndex = queueFamilyIndex;
        queueFamilyInfo.queueCount = srcFamilyProperties.queueCount;
        queueFamilyInfo.capabilities.Set(EQueueCapability::Graphics,
                                         srcFamilyProperties.queueFlags & VK_QUEUE_GRAPHICS_BIT);
        queueFamilyInfo.capabilities.Set(EQueueCapability::Compute,
                                         srcFamilyProperties.queueFlags & VK_QUEUE_COMPUTE_BIT);
        queueFamilyInfo.capabilities.Set(EQueueCapability::Transfer,
                                         srcFamilyProperties.queueFlags & VK_QUEUE_TRANSFER_BIT);
    }
}
