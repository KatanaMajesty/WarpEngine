    ...
    : m_info(bufferInfo)
{
    ...
    // might be empty, if concurrency is not required for this buffer, but if not empty, it must contain valid queue
    // types
    ...
    WARP_ASSERT(bufferInfo.memoryUsage != VMA_MEMORY_USAGE_UNKNOWN,
                "Incorrect memory usage was specified for NRI buffer");
    ...
    allocCreateInfo.memoryTypeBits =
        UINT32_MAX; // set to UINT32_MAX to let VMA find suitable memory type index based on memory requirements of this
                    // buffer and memory usage specified in allocCreateInfo
    ...
    NRI_VK_CHECK_RESULT(vmaCreateBuffer(m_info.device->GetMemoryAllocator(), &createInfo, &allocCreateInfo,
                                        &m_nativeHandle, &m_allocation, nullptr),
...
NriBuffer::~NriBuffer()
{
    ...
    {
        WARP_ASSERT(GetMemoryAllocation() != VK_NULL_HANDLE,
                    "Buffer handle is valid, but not the memory allocation handle");
...
std::byte* NriBuffer::MapMemoryToHost() const
{
    ...
    NRI_VK_CHECK_RESULT(vmaMapMemory(m_info.device->GetMemoryAllocator(), m_allocation, &mappedData),
                        "Failed to map buffer memory to host");
    return static_cast<std::byte*>(mappedData);
}

void NriBuffer::UnmapMemoryFromHost() const { vmaUnmapMemory(m_info.device->GetMemoryAllocator(), m_allocation); }

std::byte* NriBuffer::GetPersistentlyMappedMemory() const
{
    WARP_ASSERT(m_info.memoryAllocationFlags & VMA_ALLOCATION_CREATE_MAPPED_BIT,
                "NriBuffer::GetPersistentlyMappedMemory can only be called for buffers that were created with "
                "VMA_ALLOCATION_CREATE_MAPPED_BIT flag");
    VmaAllocationInfo allocInfo = {};
    vmaGetAllocationInfo(m_info.device->GetMemoryAllocator(), m_allocation, &allocInfo);
    return static_cast<std::byte*>(allocInfo.pMappedData);
}
