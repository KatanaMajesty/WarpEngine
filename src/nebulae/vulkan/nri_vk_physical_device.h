...
struct NriPhysicalDeviceInformation
{
    ...
    /// Marks the vendor of this physical device. If the vendor is not recognized, EPhysicalDeviceVendor::Unknown is
    /// set. Device vendor is uint16_t defined by PCI-SIG organization, for more information see
    /// https://pcisig.com/membership/vendor-id Device vendors are defined in "nri_pci_vendor_ids.h" auto-generated file
...
struct NriPhysicalDeviceMemoryHeap
{
    ...
    /// Memory properties flag set defines some capabilities of this memory heap, such as device locality or host
    /// visibility
...
struct NriPhysicalDeviceSurfaceProperties
{
    ...
    /// Regarding extents: On Windows platform the output surface must always be equal to the paintable window size, so
    /// min/max/curr extents will be equal.
    ...
    /// In such a case, it is not possible to create a swapchain due to the Valid Usage requirements, unless scaling is
    /// selected through VkSwapchainPresentScalingCreateInfoKHR, if supported. from
...
public:
    ...
    inline bool IsExtensionSupported(std::string_view extensionName) const noexcept
    {
        return m_supportedDeviceExtensionLUT.contains(extensionName);
    }

    inline constexpr const NriPhysicalDeviceInformation& GetInformation() const noexcept { return m_info; }
    inline constexpr const VkPhysicalDeviceMemoryProperties& GetMemoryProperties() const noexcept
    {
        return m_memoryProperties;
    }
    inline constexpr std::span<const NriPhysicalDeviceQueueFamilyInformation> GetQueueFamilyInfoArray() const noexcept
    {
        return m_queueFamilyInfos;
    }

    /// @brief A shortcut to obtain Vulkan API version supported by this physical device from
    /// NriPhysicalDeviceInformation
    ...
    WARP_NODISCARD("Multiple calls to VkPhysicalDevice on each query")
    NriPhysicalDeviceSurfaceProperties QuerySurfaceProperties(VkSurfaceKHR nativeSurface);

    /// @brief Checks whether the queue family represented by 'queueFamilyInfo' supports presentation to the surface
    /// associated with this physical device.
    bool IsSurfaceSupportedByQueueFamily(VkSurfaceKHR nativeSurface,
                                         const NriPhysicalDeviceQueueFamilyInformation& queueFamilyInfo) const;

private:
    /// @brief Queries all supported and recognized extension support (defined as EDeviceExtension enumeration)
    /// A flagset of all supported recognized extensions is formed and can be accessed by calling
    /// NriPhysicalDevice::GetSupportedExtensions
    void QuerySupportedExtensions();

    /// @brief Queries physical device generic information such as driver version/name, device name, supported API
    /// version, vendor, etc. The result is stored in NriPhysicalDeviceInformation 'm_info' field of this object and can
    /// be obtained using NriPhysicalDevice::GetInformation()
    ///
    ...
    /// The result is stored in NriPhysicalDeviceMemoryInformation field and can be obtained by calling
    /// NriPhysicalDevice::GetMemoryInformation()
    ///
    /// This call will throw a critical error on failure which will abort initialization of physical device NRI handle.
    void QueryMemoryInformation();

    /// @brief Queries all distinct queue families available to this physical device.
    /// The result is stored in an array of NriPhysicalDeviceQueueFamilyInformation represented as 'm_queueFamilyInfos'
    /// field and can be obtained by calling NriPhysicalDevice::GetQueueFamilyInfoArray()
