...
namespace Warp::nri::vk
{
    ...
    bool NriSwapchain::AcquireNextImage(VkSemaphore signalSemaphore, VkFence signalFence, uint64_t timeout) noexcept
    {
        ...
        VkResult result = vkAcquireNextImageKHR(m_device->GetNativeHandle(), GetNativeHandle(), timeout, signalSemaphore, signalFence, &m_currentImageIndex);
        switch (result)
        {
        ...
        // In such case we need to immediately handle swapchain resize and bail out of this frame render
        case VK_ERROR_OUT_OF_DATE_KHR: WARP_A_FALLTHROUGH;
    ...
    void NriSwapchain::CreateNativeSwapchain(const NriPhysicalDeviceSurfaceProperties& surfaceProperties) noexcept
    {
        ...
        WARP_ASSERT(m_numSwapchainImages >= surfaceProperties.minImageCount && m_numSwapchainImages <= surfaceProperties.maxImageCount,
        ...
        // Warp should set this value to VK_TRUE if we do not expect to read back the content of
        // presentable images before presenting them or after reacquiring them
    ...
    void NriSwapchain::QueryNativeSwapchainProperties(const NriPhysicalDeviceSurfaceProperties& surfaceProperties) noexcept 
    {
        ...
        static auto GetBestSurfaceFormat = [](const NriPhysicalDeviceSurfaceProperties& surfaceProperties) -> VkSurfaceFormatKHR
        {
    ...
    void NriSwapchain::CreateNativeSwapchainImagesAndViews() noexcept 
    {
        ...
        {
            NRI_VK_CHECK_RESULT(vkGetSwapchainImagesKHR(m_device->GetNativeHandle(), m_nativeHandle, &swapchainImageCount, nullptr),
            ...
            NRI_VK_CHECK_RESULT(vkGetSwapchainImagesKHR(m_device->GetNativeHandle(), m_nativeHandle, &swapchainImageCount, m_swapchainImageArray.data()),
        ...
        // create image views for each Vulkan image handle
        {
            ...
            for (uint32_t imageIndex = 0; imageIndex < swapchainImageCount; ++imageIndex)
            {
                ...
                NRI_VK_CHECK_RESULT(vkCreateImageView(m_device->GetNativeHandle(), &viewCreateInfo, nullptr, &m_swapchainImageViewArray[imageIndex]),
                                    "Failed to create swapchain image view at index {}",
                                    imageIndex);
            }
        }
    }
