...
public:
    ...
    bool Init(const NriDeviceCreateInfo& createInfo) noexcept;

    inline constexpr VkDevice GetNativeHandle() const noexcept { return m_nativeHandle; }
    inline constexpr VkSurfaceKHR GetSurface() const noexcept { return m_surfaceKhr; }
    inline Arc<NriPhysicalDevice> GetPhysicalDevice() const noexcept { return m_physicalDevice; }

    inline constexpr const NriPhysicalDeviceQueueFamilyInformation&
        GetQueueFamilyInfo(EDeviceQueueType deviceQueueType) const noexcept
    {
        return m_deviceQueueFamilyInfos.at(EnumValue(deviceQueueType));
    }
    inline constexpr VkQueue GetQueue(EDeviceQueueType deviceQueueType) const noexcept
    {
        return m_deviceQueues.at(EnumValue(deviceQueueType));
    }
    inline constexpr VkCommandPool GetCommandPool(EDeviceQueueType deviceQueueType) const noexcept
    {
        return m_commandPools.at(EnumValue(deviceQueueType));
    }
    ...
    {
        NRI_VK_CHECK_RESULT(vkDeviceWaitIdle(GetNativeHandle()), "Failed to wait for device to finish work");
    }

    /// @brief Allocates new ONE_TIME_SUBMIT command buffer and immediately begins it. After using
    /// NriDevice::DeallocateSingleSubmitCmdBuffer must be called
    VkCommandBuffer AllocateSingleSubmitCmdBuffer(EDeviceQueueType queueType) noexcept;
    void EndAndSubmitCmdBuffer(EDeviceQueueType queueType, VkCommandBuffer cmdBuffer) noexcept;
    void DeallocateSingleSubmitCmdBuffer(EDeviceQueueType queueType, VkCommandBuffer cmdBuffer) noexcept;

    void BeginDebugLabel(VkCommandBuffer commandBuffer,
                         std::string_view labelName,
                         glm::vec4 labelColor = glm::vec4(1.0f)) const noexcept;
...
private:
    ...
    /// From all available physical devices, only those that support required extensions and provide suitable queue
    ...
    NriSuitablePhysicalDeviceInfo
        SelectBestSuitableDevice(std::span<const NriSuitablePhysicalDeviceInfo> allSuitablePhysicalDeviceInfos);
    ...
    VmaAllocator m_memoryAllocator = VK_NULL_HANDLE; /// VMA allocator for this device, initialized on device creation
                                                     /// and destroyed on device destruction
};
