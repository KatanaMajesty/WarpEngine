...
namespace Warp::nri::vk
{
    ...
    public:
        ...
        /// Usually happens after a window resize In such case we need to immediately handle swapchain resize and bail out of this frame render
        /// 
        ...
        /// 
        /// @returns true if successfully acquired next available image, or false if swapchain properties are out of date/incompatible.
        bool AcquireNextImage(VkSemaphore signalSemaphore = VK_NULL_HANDLE, VkFence signalFence = VK_NULL_HANDLE, uint64_t timeout = UINT64_MAX) noexcept;

        /// @brief When an application's window is resized internal's of a swapchain need to be resized.
        /// When this happens NriSwapchain::HandleResize must be called in order to properly recreate underlying swapchain handles.
        /// Important note: all outstanding synchronization for NriDevice work must be done before a call to NriSwapchain::HandleResize
