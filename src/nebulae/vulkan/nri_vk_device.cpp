...
NriDevice::~NriDevice()
{
    ...
    if (m_surfaceKhr)
    {
        SDL_Vulkan_DestroySurface(m_instance->GetNativeHandle(), m_surfaceKhr, nullptr);
    }
    vkDestroyDevice(GetNativeHandle(), nullptr);
}

bool NriDevice::Init(const NriDeviceCreateInfo& deviceInfo) noexcept 
{
    ...
    requiredDeviceExtensions.push_back(
        VK_KHR_DEFERRED_HOST_OPERATIONS_EXTENSION_NAME); // required by KHR_acceleration_structure
    ...
    WARP_ASSERT(deviceInfo.window != nullptr, "Window handle is nullptr!");
    VkSurfaceKHR surfaceKhr = VK_NULL_HANDLE;
    if (!SDL_Vulkan_CreateSurface(deviceInfo.window, m_instance->GetNativeHandle(), nullptr, &surfaceKhr))
    {
        WARP_LOG_ERROR(LOGGER_NRI, "Failed to create surface from SDL window!");
        return false;
    }

    std::vector<NriSuitablePhysicalDeviceInfo> suitableDevices = QueryAllSuitableDevices(
        SuitableDeviceQueryInfo{.surface = surfaceKhr, .requiredDeviceExtensions = requiredDeviceExtensions});
    // Select the best suitable device
    {
        ...
        // also store queue family index information for every queue type, as it might be required at runtime for
        // command buffer recording, synchronization, etc. one instance of such, is VK_SHARING_MODE_CONCURRENT during
        // buffer and image creation, where we need to provide queue family indices of all queues that will be sharing a
        // particular resource
        m_deviceQueueFamilyInfos = selectedPhysicalDeviceInfo.queueFamilyInfos;
        m_surfaceKhr = surfaceKhr;

        WARP_LOG_INFO(LOGGER_NRI, "Physical device '{}' selected for NRI device creation",
                      m_physicalDevice->GetInformation().deviceName);
    }
    ...
    for (uint32_t deviceQueueIndex = 0; deviceQueueIndex < queueCreateInfoArray.size(); ++deviceQueueIndex)
    {
        ...
        // We set device-local priority to 1.0f as we instead prefer to use global-scope priorities provided by
    ...
    auto accelerationStructureFeatures =
        NRI_VK_STRUCT(VkPhysicalDeviceAccelerationStructureFeaturesKHR, &meshShaderFeatures);
    ...
    // As of Vulkan 1.2 it is recommended to provide VkPhysicalDeviceFeatures2 into pNext chain of VkDeviceCreateInfo if
    ...
    // deprecated and should not be used. Device-only layers are now marked as legacy, and Vulkan no longer
    ...
    // This field is legacy. See
    // https://registry.khronos.org/vulkansc/specs/1.0-extensions/html/vkspec.html#legacy-gpdp2. Instead provide
    // VkPhysicalDeviceFeatures2 into pNext chain of VkDeviceCreateInfo if specific features are required.
    deviceCreateInfo.pEnabledFeatures = nullptr;
    NRI_VK_CHECK_RESULT(
        vkCreateDevice(m_physicalDevice->GetNativeHandle(), &deviceCreateInfo, nullptr, &m_nativeHandle),
    ...
    for (uint32_t commandPoolIndex = 0; commandPoolIndex < m_commandPools.size(); ++commandPoolIndex)
    {
        ...
        // In Warp NRI if physical device doesn't have transfer/compute queues supported universal queue index will be
        // assigned. Because of that this code snippet potentially could invoke vkCreateCommandPool multiple times for
        // the same queue family index
        NRI_VK_CHECK_RESULT(
            vkCreateCommandPool(GetNativeHandle(), &commandPoolInfo, nullptr, &m_commandPools.at(commandPoolIndex)),
    ...
    allocatorCreateInfo.preferredLargeHeapBlockSize =
        256ull * 1024ull * 1024ull; // 256 MiB, default value recommended by VMA documentation for large heaps (>1 GiB)
    allocatorCreateInfo.pHeapSizeLimit = nullptr; // optional, we don't want to set custom heap size limits, so set to
                                                  // nullptr to use defaults (which is no limit)
    ...
    NRI_VK_CHECK_RESULT(vmaCreateAllocator(&allocatorCreateInfo, &m_memoryAllocator),
                        "Failed to create Vulkan Memory Allocator");
    return true;
}

VkCommandBuffer NriDevice::AllocateSingleSubmitCmdBuffer(EDeviceQueueType queueType) noexcept
{
    ...
    commandBufferAllocInfo.commandPool = GetCommandPool(queueType);
    commandBufferAllocInfo.level = VK_COMMAND_BUFFER_LEVEL_PRIMARY;
    commandBufferAllocInfo.commandBufferCount = 1;

    VkCommandBuffer cmdBuffer;
    NRI_VK_CHECK_RESULT(vkAllocateCommandBuffers(GetNativeHandle(), &commandBufferAllocInfo, &cmdBuffer),
                        "Failed to create single submit command buffer");

    auto beginInfo = NRI_VK_STRUCT(VkCommandBufferBeginInfo);
    beginInfo.flags = VK_COMMAND_BUFFER_USAGE_ONE_TIME_SUBMIT_BIT;
    NRI_VK_CHECK_RESULT(vkBeginCommandBuffer(cmdBuffer, &beginInfo), "Failed to begin single submit command buffer");
    return cmdBuffer;
}

void NriDevice::EndAndSubmitCmdBuffer(EDeviceQueueType queueType, VkCommandBuffer cmdBuffer) noexcept
{
    WARP_ASSERT(cmdBuffer != VK_NULL_HANDLE);
    NRI_VK_CHECK_RESULT(vkEndCommandBuffer(cmdBuffer), "Failed to end command buffer for buffer copy operations");

    auto submitInfo = NRI_VK_STRUCT(VkSubmitInfo);
    submitInfo.commandBufferCount = 1;
    submitInfo.pCommandBuffers = &cmdBuffer;
    NRI_VK_CHECK_RESULT(vkQueueSubmit(GetQueue(queueType), 1, &submitInfo, VK_NULL_HANDLE),
                        "Failed to submit queue commands");
}

void NriDevice::DeallocateSingleSubmitCmdBuffer(EDeviceQueueType queueType, VkCommandBuffer cmdBuffer) noexcept
{
    vkFreeCommandBuffers(GetNativeHandle(), GetCommandPool(queueType), 1, &cmdBuffer);
}

void NriDevice::BeginDebugLabel(VkCommandBuffer commandBuffer,
                                std::string_view labelName,
                                glm::vec4 labelColor) const noexcept
{
    ...
    if (!cmdBeginDebugUtilsLabelEXT)
    {
        cmdBeginDebugUtilsLabelEXT =
            NRI_VK_INSTANCE_PROC_ADDR(vkCmdBeginDebugUtilsLabelEXT, m_instance->GetNativeHandle());
...
std::vector<NriSuitablePhysicalDeviceInfo> NriDevice::QueryAllSuitableDevices(const SuitableDeviceQueryInfo& queryInfo)
{
    ...
    for (Arc<NriPhysicalDevice> physicalDevice : m_instance->QueryAvailablePhysicalDevices())
    {
        ...
        for (std::string_view requiredExtension : queryInfo.requiredDeviceExtensions)
        {
            ...
            {
                WARP_LOG_WARN(LOGGER_NRI, "Physical device '{}' does not support required extension '{}'. Skipping...",
        ...
        if (queryInfo.surface != VK_NULL_HANDLE)
        {
            // If valid NriSurface was provided during NriDevice creation we need to check whether it has minimal valid
            // support for this surface handle
            NriPhysicalDeviceSurfaceProperties surfaceProperties =
                physicalDevice->QuerySurfaceProperties(queryInfo.surface);
            ...
            {
                WARP_LOG_WARN(LOGGER_NRI,
                              "Physical device '{}' does not contain minimum support capabilities for surface provided "
                              "during device creation. Skipping...",
        ...
        // But some popular GPUs out there have only a single queue family with only one queue in it, so also account
        // for that
        //
        ...
        // If there are no compute-only or transfer-only queues, we will use the universal queue for those purposes as
        // well.
        //
        // Acording to NVIDIAs convention as per https://vulkan.gpuinfo.org/displayreport.php?id=43769#queuefamilies,
        // usually:
        ...
        potentialDeviceInfo.queueFamilyInfos.fill(
            NriPhysicalDeviceQueueFamilyInformation()); // initialize all queue family infos as invalid
        ...
        std::array<DeviceQueueFamilyCriteria, EnumValue(EDeviceQueueType::NumTypes)> isPerfectQueueFamily = {
            // Universal queue criteria. Checks whether a provided queue family info supports graphics, compute and
            // transfer capabilities
            [physicalDevice, nativeSurface = queryInfo.surface](
                const NriPhysicalDeviceQueueFamilyInformation& queueFamilyInfo) -> bool
            {
                return queueFamilyInfo.capabilities.AllOf(EQueueCapability::Graphics | EQueueCapability::Compute |
                                                          EQueueCapability::Transfer) &&
                       physicalDevice->IsSurfaceSupportedByQueueFamily(
                           nativeSurface,
                           queueFamilyInfo); // also make sure to check if this queue supports surface
            },
            // Transfer queue criteria. Checks whether a provided queue family info supports transfer capability but not
            // compute
            [](const NriPhysicalDeviceQueueFamilyInformation& queueFamilyInfo) -> bool
            {
            ...
            // Compute queue criteria. Checks whether a provided queue family info supports compute capability but not
            // graphics
            [](const NriPhysicalDeviceQueueFamilyInformation& queueFamilyInfo) -> bool
            {
        ...
        {
            // So for each queue family information available for this physical device we would check for each device
            // queue type whether this queue family info satisfies the criteria for that device queue type
            //
            // For more information on device queue type iteration see EDeviceQueueType enumeration definition as well
            // as 'isPerfectQueueFamily' array above
            for (uint32_t queueTypeIndex = 0; queueTypeIndex < potentialDeviceInfo.queueFamilyInfos.size();
                 ++queueTypeIndex)
            {
                NriPhysicalDeviceQueueFamilyInformation& currentQueueFamilyInfo =
                    potentialDeviceInfo.queueFamilyInfos.at(queueTypeIndex);
        ...
        // we also need to check if universal queue is present and patch transfer/compute queues if those were not found
        // iteratively
        ...
        NriPhysicalDeviceQueueFamilyInformation& transferQueueFamilyInfo =
            potentialDeviceInfo.queueFamilyInfos.at(EnumValue(EDeviceQueueType::Transfer));
        if (transferQueueFamilyInfo.IsInvalid())
            transferQueueFamilyInfo = universalQueueFamilyInfo;

        NriPhysicalDeviceQueueFamilyInformation& computeQueueFamilyInfo =
            potentialDeviceInfo.queueFamilyInfos.at(EnumValue(EDeviceQueueType::Compute));
...
NriSuitablePhysicalDeviceInfo
    NriDevice::SelectBestSuitableDevice(std::span<const NriSuitablePhysicalDeviceInfo> allSuitablePhysicalDeviceInfos)
{
    // after we have iterated over all physical devices, now we want to select one of the suitable devices to use for
    // virtual device creation To sort devices we calculate its priority based on its capabilities, such as device
    // memory size and type
    struct PhysicalDevicePriority
    {
        inline constexpr bool operator<(const PhysicalDevicePriority& other) const noexcept
        {
    ...
    static constexpr auto GetDeviceMemoryPriority = [](uint64_t totalMemoryInBytes) -> float
    {
        ...
        float totalMemoryInGB =
            totalMemoryInBytes / (1024.0f * 1024.0f * 1024.0f); // convert to float and ceil to avoid 0
    ...
    for (uint32_t i = 0; i < allSuitablePhysicalDeviceInfos.size(); ++i)
    {
        ...
        physicalDevicePriorityQueue.push(PhysicalDevicePriority{.suitablePhysicalDeviceIndex = i,
                                                                .priority = GetDeviceTypePriority(deviceInfo.type) *
                                                                                10.0f + // device type has higher weight
