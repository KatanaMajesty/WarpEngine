...
ShaderCompilerOutput
    ShaderLibrary::CompileFromLibrary(EShaderLang lang,
...
ShaderCompilerOutput
    ShaderLibrary::CompileSlangFromLibrary(const std::filesystem::path& shaderName,
                                           std::string_view entryPoint,
                                           std::span<const ShaderPreprocessorMacro> preprocessorMacros) noexcept
{
    ...
    WARP_ASSERT(shaderName.has_extension() && shaderName.extension() == SlangFileExtension,
                "Incorrect Slang shader extension of a file {}!", shaderName.string());
    ...
    // TODO: For now in Warp shader libraries we always enable validation even if WARP_DEBUG is not defined!
