...
namespace Warp
{
    ...
    enum class ELogLevel
    {
        ...
        Warn,
        Error,
        Disabled, // This value should be max, as logging is determined based on largest set enum value of log level
    };

    // A logging instance (can be created per-system)
    // 
    // An idea is to have multiple simple loggers, one for a system. All of those would be stored somewhere in a light container
    // A container would store logger objects inside of a linear array (that can be accessed using indices). Keeping track of indices - is application's responsibility
    class Logger
    {
    public:
        Logger() = default;
        ...
        template<
            ELogLevel Level,
            ELogFgColor ForegroundColor,
            ELogBgColor BackgroundColor,
            typename... Args>
            ...
            if (EnumValue(Level) >= EnumValue(m_logLevel))
            {
                ...
                if (!m_logTitle.empty())
                {
                    logTitleFormatted =
                        ...
                        std::format("{}{}<{}> {}", ForegroundColor, ELogStyle::Italic, m_logTitle, ELogStyle::Reset) : // If background color is present dont apply dim style, use foreground color
                ...
                if constexpr (IsError)
                {
                    ...
                    common::PrintLn_Err("{}{}{}{}",
                        ...
                        logTitleFormatted,
                        BackgroundColor,
                        std::format("{}[{}]: {} {}", ForegroundColor, levelPrefix, std::format(fmt, std::forward<Args>(args)...), ELogStyle::Reset));
                }
                else
                {
                    // Not an error
        ...
        {
            this->Log<ELogLevel::Trace, ELogFgColor::Bright_Black, ELogBgColor::Reset>("TRACE", fmt, std::forward<Args>(args)...);
        }

        template<typename... Args>
        void Info(std::format_string<Args...> fmt, Args&&... args) const noexcept
        {
            this->Log<ELogLevel::Info, ELogFgColor::Bright_Cyan, ELogBgColor::Reset>("INFO", fmt, std::forward<Args>(args)...);
        }

        template<typename... Args>
        void Warn(std::format_string<Args...> fmt, Args&&... args) const noexcept
        {
            this->Log<ELogLevel::Warn, ELogFgColor::Bright_Yellow, ELogBgColor::Reset>("WARN", fmt, std::forward<Args>(args)...);
        }
    ...
    enum class ELoggerType
    {
        DefaultLogger,
        NriLogger,
        NumLoggers, // this should be last and is reserved, used by crosslog::LoggerContainer
    };

    // N - number of loggers inside of a container
    //
    // Logger container is a singleton instance
    //      Motivation is coming from having a properly instanced N amount of loggers with a convenient way of retrieving them
    template<scoped_enum E, size_t N = EnumValue(E::NumLoggers)>
    ...
    using DefaultLoggerContainer = LoggerContainer<ELoggerType, EnumValue(ELoggerType::NumLoggers)>;
