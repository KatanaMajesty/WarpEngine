...
public:
    ...
    constexpr FlagSet(Enum e) { Set(e); }
    constexpr FlagSet(std::bitset<NumBits> bits)
        : m_bits(bits)
    {
    }

    constexpr UnderlyingType ToUnderlying() const noexcept { return static_cast<UnderlyingType>(m_bits.to_ullong()); }

    constexpr operator UnderlyingType() const noexcept { return ToUnderlying(); }

    constexpr FlagSet& Set(Enum e, bool v = true) noexcept
    {
        m_bits.set(EnumValue(e), v);
        return *this;
    }
    constexpr FlagSet& Flip(Enum e) noexcept
    {
        m_bits.flip(EnumValue(e));
        return *this;
    }
...
template <typename Enum>
constexpr Warp::FlagSet<Enum> operator|(Enum lhs, Enum rhs) noexcept
{
    return Warp::FlagSet<Enum>(lhs) | Warp::FlagSet<Enum>(rhs);
}
template <typename Enum>
constexpr Warp::FlagSet<Enum> operator&(Enum lhs, Enum rhs) noexcept
{
...
template <typename Enum>
constexpr Warp::FlagSet<Enum> operator~(Enum e) noexcept
{
    return ~Warp::FlagSet<Enum>(e);
}
