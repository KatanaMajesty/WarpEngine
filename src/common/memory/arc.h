...
namespace detail
{
    /// @brief ArcMarkInternal (or base-level arc mark) is an empty struct-marker for Arc
    /// pointer to "enable" Arc-pointer functionality for types derived from this mark. This
    /// should not be used directly and instead ArcMark is to be used by the client - see below
    ///
...
/// @brief ArcMark (hint-marker) is an empty struct-marker for Arc pointers that derives from
/// base-level arc mark (see struct ArcMark) but also provides some compile-time metadata on
/// allocators, basically providing some hints on how to allocate/deallocate underlying Arc
/// handle memory.
///
/// The small catch here is that with modern C++20 std-based allocators should no longer provide
/// an API to construct/destroy objects, but instead just manage underlying memory. For that
/// reason to remove "self-dependency" all allocators used by Arc **MUST** have std::byte as
/// underlying type
///
/// Also keep in mind that Arc will allocate more bytes than the client might expect when
/// creating new Arc handle. For rules of allocation read more on Arc 4 bytes are required to
/// place control block
template <typename Self, typename AllocatorHint = std::allocator<std::byte>>
    requires(std::is_trivially_copyable_v<AllocatorHint> &&
...
/// @brief Arc control block is a replacement of std::shared_ptr control block implementation to
/// allow for lower-level control of memory. Control block type can be changed when creating Arc
/// pointer if an addition layer of memory debug interface is required
class ArcControlBlock
{
public:
    static_assert(std::atomic_uint32_t::is_always_lock_free,
                  "Arc reference counter must be an always lock-free atomic!");

    ArcControlBlock() = delete; // no default constructors for control block

    /// @brief sets up a new reference count for this control block. Should only ever be used
    /// during control block initialization
    inline void SetRefCount(uint32_t value) noexcept { m_refCount = value; }

    WARP_NODISCARD("Getting ref count is an atomic operation which might affect scheduling")
    ...
    WARP_MAYBE_UNUSED inline uint32_t IncrementRefCount() noexcept
    {
        return m_refCount.fetch_add(1, std::memory_order_relaxed) + 1;
    }

    /// @returns new reference count of this control block
    /// Return result from this function **SHOULD NOT** be discarded and must instead be used to
    /// destroy the object.
    WARP_NODISCARD("Returned reference count must be used to destroy the object (if it is 0)")
    inline uint32_t DecrementRefCount() noexcept
    {
        // When decrementing the reference count, there's a potential object destruction if the
        // count hits zero thus std::memory_order_acq_rel
        uint32_t prev = m_refCount.fetch_sub(1, std::memory_order_acq_rel);
        WARP_ASSERT(prev > 0, "Arc object -> Decrement performed on instance with refcount 0! "
                              "Undefined behavior");
        return prev - 1;
    }
...
/// - If underlying type T is derived from ArcMark a default std::allocator<std::byte> will be
/// used to allocate underlying memory
/// - If underlying type T is derived from ArcMark2 an allocator specified as AllocatorHint will
/// be used instead
...
public:
    ...
    /// @brief To avoid compiler confusion a static Make function is required when creating new
    /// Arc pointers.
    ...
    Arc(const Arc<Derived>& other) noexcept
    {
        CopyInit(other);
    }
    ...
    template <typename Derived>
        requires(std::is_base_of_v<T, Derived>)
    Arc& operator=(const Arc<Derived>& other) noexcept
    {
        CopyInit(other);
        return *this;
    }
    ...
    template <typename Derived>
        requires(std::is_base_of_v<T, Derived>)
    constexpr Arc(Arc<Derived>&& other) noexcept
    {
        MoveInit(std::move(other));
    }
    ...
    template <typename Derived>
        requires(std::is_base_of_v<T, Derived>)
    constexpr Arc& operator=(Arc<Derived>&& other) noexcept
    {
        MoveInit(std::move(other));
        return *this;
    }
    ...
    WARP_NODISCARD("Getting ref count is an atomic operation which might affect scheduling")
    constexpr uint32_t GetReferenceCount() const noexcept
    {
        return IsValidReference() ? m_controlBlock->GetRefCount() : 0u;
    }

    /// @brief Checks whether this pointer holds a valid pointer and a valid reference for this
    /// pointer. Latter should always be true really, so this is just a sanity check here.
    ...
    /// Depending on whether underlying memory block pointer address is the same returns either
    /// true or false
    constexpr bool operator==(const Arc<T>& other) const noexcept
    {
        return m_allocedMemoryBlock == other.m_allocedMemoryBlock;
    }
    constexpr bool operator!=(const Arc<T>& other) const noexcept
    {
        return m_allocedMemoryBlock != other.m_allocedMemoryBlock;
...
private:
    ...
    /// @brief Releases handle of this arc pointer by decrementing internal ref count of
    /// BaseType. If after the decrement internal ref count is 0 then the object will be
    /// destroyed
    ...
    constexpr void MoveInit(Arc<U>&& other) noexcept
    {
        ...
        // check if we currently have a valid reference and if after releasing the refcount is
        // 0. if all is true then destroy the object
        if (IsValidReference())
        {
            // just release the reference - dont invalidate pointers
            ReleaseReference();
    ...
    {
        // there is currently no way to execute constexpr default-construct at a pointer, so
        // control block needs to be initialized explicitly Proposal for default_construct_at is
        // here https://www.open-std.org/jtc1/sc22/wg21/docs/papers/2021/p2283r2.pdf
    ...
    constexpr void DestroyMemoryBlock() noexcept
    {
        ...
        // it is dangerous to have throwable destructors overall as it causes std::terminate on
        // almost all compilers (whereas others might even be UB)
        static_assert(std::is_nothrow_destructible_v<T>,
                      "Destructors MUST be marked with nothrow to be used with Arc!");
    ...
    /// This is to handle empty-struct cases where control block + sizeof(T) would result in 4 +
    /// 1 bytes allocated
    static constexpr size_t AllocationWordAlignment = sizeof(uint32_t);
    static constexpr size_t AllocationSizeInBytes =
        sizeof(ArcControlBlock) + AlignUp(sizeof(T), AllocationWordAlignment);
    static constexpr size_t AllocationHandleOffset = sizeof(ArcControlBlock);

    // It is assumed that all pointers will properly be constructed after the call to this
    // mem-fn Should qualify for constexpr since C++20 if allocator is std-based (which we
    // assume it to be)
    constexpr void AllocateMemoryBlock() noexcept
    {
        ...
        // according to alloc rules second in mem layout comes T at offset defined as
        // sizeof(ControlBlock)
        m_ptr = reinterpret_cast<T*>(m_allocedMemoryBlock + AllocationHandleOffset);
    }

    // If Arc pointer is out-of-scope but lifetime should still be preserved - dont deallocate
    // memory block but still invalidate pointers
    ...
    /// We always guarantee control block to be first in memory layout, so we can treat it as a
    /// beginning of underlying allocation block - prefer to unionise it here.
    union
    {
